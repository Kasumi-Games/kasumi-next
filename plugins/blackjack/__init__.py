...
from .help_render import help_page  # noqa: E402
...
async def handle_start(event: MessageEvent, arg: Optional[Message] = CommandArg()):
    ...
    try:
        ...
        # ``renderer`` owns the expensive card art loaded at startup.  Fork a
        # tiny per-game view with this player's kit instead of changing the
        # shared renderer: separate games may be awaiting input concurrently.
        kit = kit_for_user(event.get_user_id())
        session_renderer = renderer.for_kit(kit)
        ...
        # every table/hand render; the renderer never resolves identity. The
        # avatar is fetched once here — the cache makes per-action renders
        # cheap — and None keeps the initial-badge fallback.
...
async def handle_stats(event: MessageEvent):
    ...
    try:
        ...
        identity = identity_for(user_id, avatar=await get_avatar(user_id))

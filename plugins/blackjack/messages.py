...
class Messages:
    BET_PROMPT = "你要下注多少Pt呢？"
    BET_TIMEOUT = "时间到了哦，黑香澄流程已结束"
    BET_INVALID = "输入的金额不是数字，请重新输入"
    BET_TOO_SMALL = "下注Pt不能少于 1 个哦，请重新输入"
    BET_NOT_ENOUGH = "你只有 {amount} 个Pt，不够下注哦~"
    ...
    DOUBLE_NOT_ENOUGH = "你只有 {amount} 个Pt，不够双倍下注哦~请重新选择"

    TIMEOUT_LOSE = "时间到了哦，游戏自动结束。下注的Pt已没收哦~\n"

    BUST_LOSE = "你爆牌啦，Kasumi 获胜！输掉了 {amount} 个Pt"
    DEALER_BUST_WIN = "Kasumi 爆牌，你获胜啦！赢得了 {amount} 个Pt"
    RESULT_WIN = "{player} > {dealer}，你获胜啦！赢得了 {amount} 个Pt"
    RESULT_LOSE = "{player} < {dealer}，Kasumi 获胜！输掉了 {amount} 个Pt"
    ...
    SPLIT_NOT_ENOUGH = (
        "你只有 {amount} 个Pt，不够分牌的额外下注哦~接下来将按照不分牌处理"
    )

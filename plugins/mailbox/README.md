...
/mail
```
以图片卡片显示当前用户的邮件列表，包括未读/已读状态、奖励信息和过期时间。
邮件较多时自动切换为单行密集排版，不分页。
...
/mail <编号>
```
读取指定邮件的正文，自动领取附带的奖励，并以卡片展示领取结果。
所有卡片只显示序号（一键领取明细里的编号就是它，领取不改排序，
编号可以直接回看）。`M<邮件ID>` 是历史遗留的代码写法，仍被接受但
不再显示。
...
/邮件 claim
```
一次领取所有**带附件**的未读邮件，返回一张汇总卡片（合计获得 + 每封明细）。
无附件的公告不会被领取，保持未读，卡片会说明还剩几封未读通知。
...
#### 创建定时邮件
```bash
/schedulemail add -r <接收者> -w <预定时间> -e <过期天数> -k <星之碎片> -s <星星贴纸> -t <标题> -c <内容> [--name 自定义名称]
```
...
### 维护补偿邮件
```bash
# 创建维护补偿邮件（绝对时间，含星之碎片和星星贴纸）
/schedulemail add -r all -w "2024-01-15 18:00" -e 7 -k 100 -s 50 -t 维护补偿 -c "感谢大家的耐心等待，维护已完成！"
...
### 星之碎片/贴纸奖励建议
- **日常签到补偿**: 10-50 碎片
- **维护补偿**: 50-200 碎片 + 50-200 贴纸
- **活动奖励**: 100-1000 碎片 + 100-500 贴纸
- **特殊贡献奖励**: 500-2000 碎片 + 200-1000 贴纸

...
- **头像框**：规格文档 `docs/design/avatar-frame-spec.md`（512×512、中央 Ø416 透明圆、笔触下限、
  主题配色参考）；头像框现在是独立物品资源，**用户手绘稿覆盖
  `plugins/inventory/resources/items/avatar_frames/<item_id>.png` 即生效，零代码改动**。
...
### 阶段 5：赛季生命周期加固 ✅（2026-07-28）

- 开季成为可审计的一次性状态转换（`opened_at`），`starting_points` 按赛季 scope
  懒初始化且幂等；显式 `now` 会贯穿配置同步与状态刷新。
- 结算增加状态门禁：未来赛季不能结算、提前结算必须 `--force`、已结算赛季禁止重算；
  已结算配置冻结，不再因后续 JSON 修改而改写历史排名规则。
- 排名与奖励先在库存库原子落盘，邮件作为 durable outbox 后续投递；
  邮箱新增唯一 `external_key`，覆盖「邮件已提交但确认丢失」与「邮箱暂时不可用」两种恢复路径。
  调度器每分钟自动重试，管理员可用 `/season-admin retry-rewards` 手动补投。
- 配置同步新增硬校验：时区偏移、时间正序、赛季不重叠、编号/key 唯一、初始 Pt、
  快照间隔/名次、奖励数量与排名区间。
- 提前结算后使用稳定的休赛期 scope，不会在原定结束日再次发放休赛期初始 Pt。
- 管理员可用 `/season-admin preview <season_key>` 在结算前只读核对钱包、参与人数和预计邮件数。
- 旧库存/邮箱数据库启动时自动补 `opened_at`、`season_key` 唯一索引和邮箱 `external_key`。
- 旧余额迁移在赛前只延后、不扫描旧库；赛季开始后由生命周期任务自动续跑，无需重启。
  已用 25,168 名真实玩家的只读备份副本完成赛前→开季→重复迁移演练，逐玩家和总量均一致。
- 回归基线：**610 passed + 317 subtests**；5,000 钱包隔离结算从约 0.84s 降至 0.22s。
